...
在使用[%(text-en)Jekyll%]的时候出了各种问题。第一个问题是[%(text-en)Liquid%]出错，提示信息里面有包含[%(text-en)pygments%]，于是自己重新装了以下[%(text-en)pygments%]，问题依旧，然后发现问题在于[%(text-en)Jekyll%]的版本——不知道中间捣鼓的时候怎么装了3个版本上去，卸载一个版本就好了[3]。第二个问题，也是导致我重装无数次[%(text-en)Ruby%]的问题，就是用[%(text-en)Jekyll%]生成的页面里面出问题，我在[@_config.yml@]中设置的[@excerpt_separator@]完全不起作用了——然而在[%(text-en)Windows 7%]下面都是正常的，为了解决这个问题，我尝试了
